...
#define YT100_DISPLAY_TEST 1   // 1 = OLED test only, 0 = normal tuner firmware
...
void main() {
...
#if YT100_DISPLAY_TEST
   while(1) {
      LATA2_bit = ~LATA2_bit;          // RA2 LED heartbeat
   ...
   if(Button(&PORTB, 0, 100, 0))  { //  Fider loss input
      ...
      //
      while(1) {
         if(Button(&PORTB, 2, 50, 0)){   // BYP Tune_Button
            if(Fid_loss<99) {
               Fid_loss ++;
               show_loss();
         ...
         //
          if(Button(&PORTB, 1, 50, 0)){   // AUTO Tune_Button
            if(Fid_loss>0) {
               Fid_loss --;
               show_loss();
               EEPROM_Write(0x34, Dec2Bcd(Fid_loss));
...
//***************** Routines *****************
 ...
 void Tune_Button_proc_test(void) {
    if(Button(&PORTB, 0, 50, 0)){    // Tune btn
        Delay_ms(250);
        asm CLRWDT;
        if(PORTB.B0==1) { // short press Tune_Button
           if(SW==0) SW = 1; else SW = 0;
           set_sw(SW);
           lcd_ind();
        }
        else {   // long press Tune_Button
           if(L==1) L = 0; else L = 1;
           if(L==1) {
     ...
     }   // end of BYP Tune_Button
     //
     if(Button(&PORTB, 1, 50, 0) & Bypas==0){   // Auto Tune_Button
        asm CLRWDT;
        while(PORTB.B1==0) {
           if(L & ind>0) {
              ind --;
...
void Tune_Button_proc(void) {
    ...
    if((Button(&PORTB, 0, 50, 0) & tune_btn_release) | Soft_tune){
        ...
        Delay_ms(250);
        asm CLRWDT;
        if(Soft_tune == 0 & PORTB.B0==1) { // short press Tune_Button
           show_reset();
           bypas =0;
        }
        else {  // long press Tune_Button
...
void lcd_pwr() {
   ...
   if(Auto & SWR_fixed>=Auto_delta & ((SWR_fixed>SWR_fixed_old & (SWR_fixed-SWR_fixed_old)>delta) | (SWR_fixed<SWR_fixed_old & (SWR_fixed_old-SWR_fixed)>delta) | SWR_fixed_old==999))
      Soft_tune = 1;
   //
   if(PORTB.B1==0 | PORTB.B2==0 | (PORTB.B0==0 & tune_btn_release)) {Tune_Button_delay(); if(but==1) {but = 0; return;} }   // Fast return if Tune_Button pressed
   //
