...
void set_addressing (char pagenum, char c_start) {
  ...
  Soft_I2C_Write(0);              // command stream
  Soft_I2C_Write(0xB0 + pagenum); // page
  Soft_I2C_Write(a);              // lower column nibble
  Soft_I2C_Write(0x10 + b);       // higher column nibble
  Soft_I2C_Stop();

  // Data phase
  Soft_I2C_Start();
  Soft_I2C_Write(led_addr);
  Soft_I2C_Write(64);             // data stream
}
//
...
//
 ...
 void Soft_I2C_Init () {
    ...
    // YT-100:
    // OLED is on RC6/RC7.
    // Software I2C uses TRIS as open-drain:
    // LAT=0 is prepared once.
    // TRIS=0 pulls the line low.
    // TRIS=1 releases the line; the external pull-up makes it high.
    //
    // led_type==0 is intentionally kept for a future no-display/status-LED variant.
