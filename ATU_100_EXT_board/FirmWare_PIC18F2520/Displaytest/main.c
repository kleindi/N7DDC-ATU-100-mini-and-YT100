// PIC18F2520 / mikroC PRO for PIC
// OLED SSD1306 I2C an RB6/RB7
// OLED SSD1306 I2C an RC6/RC7
// SCL = RC6, SDA = RC7
...
void i2c_write(unsigned char d) {
   ...
   // ACK-Takt, ACK wird hier nicht ausgewertet
   sda_high();
   i2c_delay();
   scl_high();
   i2c_delay();
...
void oled_cmd(unsigned char c) {
   i2c_start();
   i2c_write(OLED_ADDR);
   i2c_write(0x00);   // Control byte: commands
   i2c_write(c);
   i2c_stop();
}
...
void oled_test_pattern() {
   ...
   // Muster 3: horizontale Streifen
   oled_fill(0xAA);
   Delay_ms(1000);

   // Muster 4: invertierte horizontale Streifen
   oled_fill(0x55);
   Delay_ms(1000);

   // Muster 5: vertikale Streifen
   for(page = 0; page < 8; page++) {
      oled_set_pos(page, 0);
