...
void btn_push() {
   ...
   tune();
    if(type==0) {
       // no display, no status LED on YT-100
    }
   ...
   if(swr!=SWR_old) {
      ...
      if(SWR==0) {  // Low power
         ...
         if(type==4 | type==5) led_wr_str (2, 16+4*12, work_str_2, 4);  // 128*64 OLED
            ...
            if(swr<=150) { Soft_I2C_Scl = 0; Soft_I2C_Sda = 1; } // Green
            else if(swr<=250) {Soft_I2C_Scl = 0; PORTB.B7 = 0;} // Orange
            else { Soft_I2C_Scl = 1; Soft_I2C_Sda = 0; }  // Red
         }
      }
   }
   asm CLRWDT;
